...
%% Správné nastavení mezer před sekcemi
%\usepackage[setspace]
%\onehalfspacing
...
%\raggedbottom % nehlídá stejnou velikost stránek -> neroztahuje mezery mezi obrázky apod. X dává pozn. pod čarou nahoru

\usepackage[dvips]{graphicx}    % Balíček 'graphicx' pro vkládání obrázků
\usepackage{color}% pro svg obrázky
...
\usepackage{float} % pro umístění obrázků apod.

\usepackage{textcomp}

\usepackage{amsmath} % pro aligned
\addtolength{\jot}{1em} % 
...
%\setlist{noitemsep} % bez mezer mezi položkami
\setlist{\itemsep=0.2em,\topsep=0.5em,\parsep=0em, \parskip=0em} % nastaveni mezer mezi položkami a před seznamem -- NEFUNGUJE!!
...
%\title{Tvorba QGIS zásuvného modulu pro slučování vektorových map}
